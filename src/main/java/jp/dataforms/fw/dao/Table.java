...
public class Table {
				...
				if (m == null) {
					...
					if (!colname0.equals(colname1)) {
						...
						if (!type0.equals(type1)) {
							logger.warn(() -> this.getTableName() + ":column " + colname0 + " type missmatch.(" + type0
									+ "," + type1 + ")");
							return false;
						}
					}
	...
	public boolean structureAccords(final Dao dao) throws Exception {
		...
		if (rel != null) {
			...
			if (fkset.size() != fklist.size()) {
				logger.info(() -> "table " + this.getTableName() + " foreign key count missmatch.(" + fkset.size() + ","
						+ fklist.size() + ")");
				return false;
			}
	 ...
	 * @return 結合条件。
	 */
	public final String getLinkFieldCondition(final Table table, final String field, final Table joinTable,
			final String alias, final String linkField) {
		StringBuilder sb = new StringBuilder();
